...
description: "eBPF 只能跑在 Linux kernel，如果你用的是 Mac。這篇給你一個乾淨、可重複的做法：開一台真正的 Linux VM（含 CO-RE 需要的 BTF），附一鍵安裝腳本，也說清楚為什麼別用 Docker Desktop。"
date: 2026-06-18 11:00:00 +0800
categories: [eBPF, 環境]
tags: [ebpf, macos, linux, devops]
render_with_liquid: false
image:
  path: /assets/img/ebpf-on-mac-zh.png
  alt: "Mac →（VM）→ Linux kernel：eBPF 只能跑在 Linux kernel，用一鍵安裝腳本在 Mac 上開好含 BTF 的 Ubuntu VM"
---
...
- **Docker Desktop 的 LinuxKit kernel** 常常**沒有**。❌——這就是在容器裡試 eBPF，結果 `vmlinux.h` 失敗這麼常見的原因。
...
如何選擇？
**快速上手？用 Multipass。** 想讓檔案「直接就在 VM 裡」？用 Lima。
...
![在 Lima VM 裡用 bpftrace 即時追蹤 execve()：另開一個 shell 跑 ls，立刻被 trace 到](/assets/img/bpftrace.png)
...
- 安裝腳本 + `hello-ebpf` 範例：[github.com/hyperredstart/hello-ebpf](https://github.com/hyperredstart/hello-ebpf)
- Multipass 文件：[multipass.run](https://multipass.run)
...
> 🌐 English version (dev.to)：[eBPF on a Mac: A 5-Minute Linux VM with Multipass or Lima](https://dev.to/hyperredstart/ebpf-on-a-mac-a-5-minute-linux-vm-with-multipass-or-lima-5dng)
